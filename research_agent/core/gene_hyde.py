...
class GenStatementHyde:
    ...
    def __init__(self, base_path: Optional[str] = None, max_concurrent: int = 15):
        ...
        prompt_file = base_path / "gene_statement_hyde.jinja"
        try:
    ...
    async def generate_statement_hyde(self, statements: str, keywords: str, max_retries: int = 3) -> List[str]:
        """
        根据给定的语句调用LLM生成相关的引用。

        :param statements: 研究的主题或文本内容
        :param max_retries: 最大重试次数
        :return: 模型返回的引用语句列表
        """
        prompt_messages = self._prepare_prompt_messages(statements, keywords)

        for attempt in range(max_retries):
            try:
                response = await self.llm.completion(prompt_messages)
                response_data = json_repair.loads(response)
                return response_data.get("abstract", []), response_data.get("core_question", [])
            except Exception as e:
    ...
    def _prepare_prompt_messages(self, statements: str, keywords: str) -> List[dict]:
        """
        准备生成查找语句引用所需的提示消息。

        :param statement: 文本内容
        :return: 包含系统和用户提示信息的字典列表
        """
        system_prompt = self.prompt_template.render(
            role="system", statement=statements)
        user_prompt = self.prompt_template.render(
            role="user", statements=statements, keywords=keywords)
